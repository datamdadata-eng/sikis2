...
export async function ensureTaleplerTable(): Promise<void> {
  ...
  try {
    ...
    `);
    await client.query(
      "ALTER TABLE talepler ADD COLUMN IF NOT EXISTS para_kime_gitti VARCHAR(255) NOT NULL DEFAULT ''"
    );
    await client.query(
      "ALTER TABLE talepler ADD COLUMN IF NOT EXISTS acici_kim VARCHAR(255) NOT NULL DEFAULT ''"
    );
    await client.query(
      "ALTER TABLE talepler ADD COLUMN IF NOT EXISTS kapatici_kim VARCHAR(255) NOT NULL DEFAULT ''"
    );
    await client.query(
